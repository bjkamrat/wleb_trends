...
for(j in 1:nrow(gen_dat_2)){
    ...
    eList_sen <- setPA(eList, paStart = 12, paLong = 3)
...
delta_conc$season <- "winter"

delta_flux <- as.data.frame(do.call(rbind,flux))
delta_flux$type <- "flux"
delta_flux$season <- "winter"

# Lets write some table to mess around with in excel
# annual values
write.csv(annual, file=paste("data/process/seasonal/winter/",poi,"_overall_results.csv",sep = ""),
            row.names = FALSE)

# concrentration changes
write.csv(delta_conc, file=paste("data/process/seasonal/winter/",poi,"_conc_results.csv",sep = ""),
            row.names = FALSE,quote=FALSE)

# flux changes
write.csv(delta_flux, file=paste("data/process/seasonal/winter/",poi,"_flux_results.csv",sep = ""),
            row.names = FALSE,quote=FALSE)
